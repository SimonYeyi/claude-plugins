...
| 情况 | 处理 |
|------|------|
| 测试用例评审通过 | 编写测试代码（单元+平台） → 运行测试验证 → 生成测试报告 → **请求** 主控 dispatch **test-reviewer** 进行测试代码及报告评审，传递评审类型标记：测试代码及报告评审 |
| 测试代码及报告评审通过 | 上报测试流程结束 |
| 测试用例评审有意见，count < 5 | 修复/反驳 → **请求** 主控重新 dispatch 评审Agent，传递评审类型标记：测试用例评审 |
| 测试代码及报告评审有意见，count < 5 | 修复/反驳 → **请求** 主控重新 dispatch 评审Agent，传递评审类型标记：测试代码及报告评审 |
| 测试用例评审/测试代码及报告评审有意见，count = 5 | 汇总分歧上报主控 |
| 测试用例评审 count = -1 | 执行决断 → 测试用例评审通过 |
| 测试代码及报告评审 count = -1 | 执行决断 → 上报测试流程结束 |

### 处理功能代码修复反馈
**输入**：开发Agent的修复反馈
**处理**：执行测试代码 → 生成测试报告 → **请求** 主控 dispatch **test-reviewer**，传递评审类型标记：测试代码及报告评审
